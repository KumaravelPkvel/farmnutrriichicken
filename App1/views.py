...
def closing_stock_view(request):
    ...
    stocks = ClosingStock.objects.values('date', 'day').distinct()
...
def dailysheet(request):
    if request.method == 'POST':
        if 'delete' in request.POST:
            ...
            try:
                report_date = datetime.strptime(report_date, "%Y-%m-%d").date()
                DailySheet.objects.filter(date=report_date).delete()
            except ValueError:
    ...
    # Retrieve all distinct date and day combinations
    stocks = DailySheet.objects.values('date', 'day').distinct()
...
def daily_sales(request):
    if request.method == 'POST':
        if 'delete' in request.POST:
            ...
            try:
                report_date = datetime.strptime(report_date, "%Y-%m-%d").date()
                DailySales.objects.filter(date=report_date).delete()
            except ValueError:
    ...
    # Retrieve all distinct date and day combinations
    stocks = DailySales.objects.values('date', 'day').distinct()
...
def weekly_report(request):
    if request.method == 'POST':
        if 'delete' in request.POST:
            ...
            try:
                report_date = datetime.strptime(report_date, "%Y-%m-%d").date()
                WeeklyReport.objects.filter(date=report_date).delete()
            except ValueError:
    ...
    # Retrieve all distinct date and day combinations
    stocks = WeeklyReport.objects.values('date', 'day').distinct()
